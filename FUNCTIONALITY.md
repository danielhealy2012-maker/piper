...
| Custom avatars | ✅ | Phase 2 #11: "make my avatar a cartoon fox" / "give Sam a robot avatar" (`generateAvatar`, optional `authorId`). SHARED, unlike nicknames — an avatar is real identity, so it's a new `avatar_url` column on the existing `profiles` table (**needs migration 0006**) rather than a personal table, reusing profiles' existing RLS as-is. Either participant may set either avatar, no approval step — same trust model as reactions/shared components — but `api/avatar.js` verifies caller and target share a conversation before writing, so this can't be used to deface an arbitrary stranger's profile. Reuses the Replicate/Flux pipeline from background generation (`api/_lib.js`'s `generateImageBuffer`, factored out of `api/image.js`) but the client never gets direct write access to `avatar_url` at all — only `api/avatar.js` (service role) sets it, after generating the image itself, matching the same "model authors a prompt, server owns the URL" trust boundary as `wallpaperUrl`. Renders next to every bubble (both directions), not just incoming. Fixed three real bugs from live testing: `profiles` changes had no Realtime signal at all — client-side listener wired into `subscribeConversation`, but `profiles` also had to be added to the `supabase_realtime` publication itself (**needs migration 0007** — the table was simply never published, so no client-side code could have made this live without it, only a manual reload ever picked up the change); and the avatar style suffix said "portrait", which biased the image model toward rendering a person even for literal graphic requests like a flag (dropped "portrait", and the router now writes literal graphic descriptions for flags/logos/symbols instead of a bare name). Not available in local dev, same as background generation |
...
| Feature | Status | Details |
|---------|--------|---------|
| Create conversation | ✅ | Auto-create on first sign-in; **"+ New chat"** creates additional ones on demand — see Multiple conversations below |
| Rename conversation | ✅ | `updateConversationTitle`, persisted in DB |
| Multiple conversations per user | ✅ | Replaced `getOrCreateConversation` (always returned your MOST RECENT conversation) with `createConversation` (always creates a fresh one) + `listMyConversations`. Tabs can be closed (✕) via `leaveConversation` — removes only YOUR OWN membership, never the conversation or its messages, so the other participant is unaffected (**needs migration 0008** — no delete policy existed on `conversation_members` at all before this, so closing a tab was previously impossible). Closing your last tab immediately creates a fresh one rather than leaving you with zero. The old behavior had a real bug: sending your one existing invite link to a NEW friend silently added them to your conversation with the FIRST friend instead of giving them their own — the app only ever had one conversation to hand out, so "start a new chat with someone else" was structurally impossible, and the header's "who am I talking to" logic (arbitrary first non-self match) would then start showing the wrong person. Now every "+ New chat" gets a genuinely fresh conversation, own invite_code, starts with just you until someone uses that specific link — so ordinary use never produces an accidental 3-person room. A tab bar (`App.tsx`'s `ConversationTabs`) switches between them, labeled by the other participant's name (or the conversation's custom title if renamed, or "no one's joined yet"). Last-viewed conversation persists in localStorage so a reload doesn't dump you back on whichever is most recent. Switching tabs fully remounts `Workspace` (via React `key`) rather than trying to reconcile onto it — it holds too much per-conversation state (messages, spec, undo stack, log) to safely transition in place. Does NOT fix pre-existing 3+-person conversations (if you already sent one link to two different people before this shipped) — that's still the group-chat gap, unchanged |
...
| Conversational back-and-forth | ✅ | The instruction panel is one unified chat window with Piper — header, scrollable thread, contextual panels, and the composer all inside a single bordered container (not a form floating separately above an unrelated-looking log box). The old grab-bag of ~11 example-instruction chips is gone, replaced with a single explicit "Reset to default" button. Your asks + Piper's replies render as bubbles, not a flat log. Three distinct outcomes beyond plain success/fail: a lowered clarification threshold — vague pushback like "I don't like it" or "that's not right" now asks what specifically instead of guessing again; and a new third mode, genuine Q&A ("why does it look like that", "what other colors could work") answered conversationally (`feasible:true`, no action taken, not a failure) rather than forced into an action or declared infeasible. Backed by the existing 8-turn session memory (`history.ts`) — session-only by design, no new persistence |
