...
export function buildSystemPrompt(): string {
  ...
  return [
    ...
    "   - Keep it robust: clean up every `setInterval`/`setTimeout` in a `useEffect` cleanup function so it doesn't run forever after the user moves on; keep visual size compact (it renders inside a small pill or a slim strip, not full-screen); avoid unbounded loops.",
