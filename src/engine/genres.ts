...
export const SPECIALIST_SECTIONS: SpecialistSection[] = [
  ...
  },
  {
    ...
    build: () => [
      ...
      "   - If ambientEffect is ALSO part of this request (an animated/moving version of the subject — \"an animated whale background\", \"a background with a swimming fish\"): the generated image itself can only ever be static, so the motion has to come from customEffects.onLoad creating a real moving element (an emoji or a simple drawn shape representing the subject, e.g. 🐳 for a whale) layered on top via container.appendChild, with its own infinite CSS animation, exactly as documented in the customEffects section. This combination — a real generated illustrated background PLUS a moving decorative element on top — genuinely delivers what was asked; it is not a compromise, so leave `limitation` null for it (aside from the normal image-generation-failure case already covered above). DO actually set `backgroundImagePrompt` to generate a real matching scene (e.g. for a whale, a proper illustrated ocean/underwater scene) — do not take the cheaper way out of leaving it null and falling back to a flat color or one of the 8 fixed scenes just because the moving element is handled separately; the generated background is exactly what makes this look intentional instead of a plain color with an emoji floating on it. Do NOT try to make the background image itself move, and do NOT explain this as a limitation of the 8 fixed scenes — that's a different, unrelated constraint that has nothing to do with why a picture can't move on its own.",
    ],
  },
];
