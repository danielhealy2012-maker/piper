...
export function buildRouterPrompt(): string {
  ...
  return [
    ...
    '  - {"kind":"generateAvatar","prompt":<string>,"authorId":<id, optional>} — generate a new profile picture. Omit authorId (or use the current user\'s own id) for "make MY avatar X". Set authorId to the OTHER participant\'s exact id from the participants list for "make SAM\'s avatar X" / "give Sam a robot avatar" — this is allowed, same as reacting to Sam\'s message or editing the shared theme; there is no approval step. You write the image generation prompt from their description — write it LITERALLY: for a flag, logo, or symbol, describe the actual graphic ("the flag of Japan: a red circle centered on a white field") — do NOT describe a person, portrait, or scene, which is what a vaguer prompt like "flag of Japan" alone tends to produce. For a creature/character/object, describe that thing directly. Examples: "make my avatar a cartoon fox", "give Sam a robot avatar", "make my avatar the flag of Japan", "change Sam\'s profile picture to a mountain landscape". This is a real AI-generated image, distinct from the appearance/theme system — route avatar requests here, never through themeInstruction.',
    "",
